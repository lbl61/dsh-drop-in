...
把系统文件管理器里的文件直接拖进 DeepSeek Harness Web 界面。文件会显示在输入框上方的文件栏里，或以内联 `@文件名` 形式插入到正在输入的文字中；发送消息时随消息一起发出（含 **绝对路径**），并在气泡中渲染成文件卡片。助手读取的是真实路径——不上传、不复制内容（浏览器兜底时小文件会复制到工作区）。
...
- ✏️ **@ 内联引用**：输入框有焦点时拖入/粘贴文件，会在光标处插入 `@文件名`（如 `你好@报告.zip看一下这个文件`）；发送时自动转为 `@[文件名](绝对路径)` 内联格式，聊天气泡中渲染为文件卡片，助手可直接按路径读取
- 🖼️ **剪贴板粘贴**：在输入框粘贴图片/文件走同一管道（@插入或文件栏），不再落入官方原生图片附件轨
...
2. 输入框有焦点 → 文件以 `@文件名` 插入到光标处；否则出现在输入框上方的文件栏
3. 输入消息并发送（回车 / Ctrl+Enter / 发送按钮均支持）
4. 消息气泡中显示文件卡片——助手在消息里直接拿到绝对路径（`@[文件名](路径)` 或 `📎 拖入文件` 块），可用自己的工具读取文件

## 工作原理

- 客户端半在捕获阶段拦截文件拖拽（官方图片上传流程不会抢走事件），维护按会话隔离的文件栏（`conversation.input.dock`），用文件卡片渲染用户消息气泡（`conversation.chat.node` 的 `user` key），并在提交前把 `📎 拖入文件` 块拼进 draft（回车、Ctrl+Enter 和发送按钮三条路径都覆盖）；输入框有焦点时改为在光标处插入 `@文件名` 并在发送时转为 `@[文件名](绝对路径)`
