window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      const tokensBySession = new Map(); // sid -> Map<"@name", entry> 输入框内联 @ 引用
      ...
      /** @ 模式：合成器输入框有焦点时在光标处插入 @文件名，并登记 token；返回是否插入。 */
      ...
      const onKeyDown = (ev) => {
        ...
        if (getEntries(currentSessionId).length === 0 && !hasTokens()) return;
      ...
      const Dock = (props) => {
        ...
        React.useEffect(() => {
          ...
          if (props.inputActions && typeof props.inputActions.submit === "function") {
            ...
            const wrapped = () => {
              ...
              if (getEntries(sid).length === 0 && !(tmap && tmap.size > 0)) { orig(); return; }
              const el = findComposerTextarea();
        ...
        return React.createElement("div", { className: "ddrop-dock" }, [
          ...
          React.createElement("div", { className: "ddrop-hint", key: "__hint" }, "输入框有焦点时拖入/粘贴文件会在光标处插入 @文件名（发送时转为含路径的引用）；否则显示在此栏，随消息一起发送")
        ]);
      };
      const Overlay = () => {
      ...
      const SettingsRow = () => {
        ...
        return React.createElement("div", { className: "ddrop-settings" }, [
          ...
          React.createElement("p", { key: "d", className: "ddrop-settings-desc" }, "从系统拖入或粘贴的文件会显示在输入框上方（chips）或以 @文件名 插入到文字中，发送时随消息附带（含绝对路径）。关闭后拖拽/粘贴回落到内置的图片行为。")
        ]);
      };
