window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      /** 类型标识（大写扩展名，无扩展名显示"文件"）——chip 空间小，类型放最前保证可见。 */
      const typeOf = (name) => {
        const ext = String(name).split(".").pop();
        if (!ext || ext === name) return "文件";
      ...
      const bailInsertRef = (sid, entry, span) => {
        ...
        const reference = {
          source: "drop-in",
          ref: entry.id,
          // 类型在前：原生 chip 单元格只有 4em 宽，label 被裁剪时类型仍可见
          label: typeOf(entry.name) + " " + entry.name,
       ...
       *  不要求输入框有焦点（外部拖入时 activeElement 常不在输入框）——无焦点插到草稿末尾。 */
      const insertRefAtCaret = async (entries, sid) => {
        const el = findComposerTextarea();
        if (!el || el.disabled || el.readOnly) return false;
        if (!el.isConnected || el.offsetParent === null) return false; // 合成器被接管/隐藏时不插入
        loadRefs();
        const focused = document.activeElement === el;
        let ok = false;
        let pos = Math.min(Math.max(focused ? (el.selectionStart ?? 0) : 0, 0), String(el.value || "").length);
        if (!focused) pos = String(el.value || "").length;
      ...
      /** 气泡内联解析：@[名称](路径) → 文件卡片片段。路径可含括号（如 ...main (1).zip）。 */
      const splitInline = (text) => {
        const segments = [];
        // 括号组整体消费：路径里的 "(1)" 不会被误判为结尾
        const re = /@\[([^\]]+)\]\(((?:[^()]|\([^)]*\))*)\)/g;
        let last = 0;
        let m;
      ...
      ctx.effect(() => {
        ...
        styleEl.setAttribute("data-dropin-css", "1");
      ...
      const BubbleCard = (props) => {
        ...
        return React.createElement("div", { className: "ddrop-chip", title: e.path || e.name }, [
          ...
          React.createElement("span", { className: "ddrop-type", key: "t" }, typeOf(e.name)),
      ...
      // agent 运行中发送（不打断）的消息是 steering 节点——同样接管渲染，否则 @ 引用被官方文本处理吃掉
      ctx.slots.inject("conversation.chat.node", () => ctx.slots.register({
        name: "conversation.chat.node",
        key: "steering",
        priority: -10
