 ...
 * - 四个仅限回环地址的 POST 路由（/api/dsh-drop-in/*）承接页面端的上报/移除/清空/文本兜底复制；
...
function apply(ctx, config) {
  ...
  const post = (path, fn) => ctx.webServer.register({
    kind: "exact",
    path,
  ...
  ctx.effect(() => {
    ...
    const tool = defineTool({
      name: "dropped_files",
      description: "列出用户最近拖入聊天窗口、尚未随消息发送的文件（含每个文件的绝对路径）。用户拖入文件后文件会显示在输入框上方；发送消息时文件信息会作为“📎 拖入文件”块自动附加到消息里（含绝对路径），并在气泡中渲染为文件卡片。若消息里已有该块，则无需调用本工具；仅在需要查证或文件未随消息发送时调用。无参数。",
      parameters: {},
      output: {
        schema: {
          type: "object",
